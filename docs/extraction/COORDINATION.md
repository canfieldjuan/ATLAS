...
Last updated: 2026-05-03T22:45Z by claude-2026-05-03-b
...
|---|---|---|---|---|---|
| `extracted_llm_infrastructure` | 2 (standalone toggle landed; Phase 3 decoupling pending) | #87 | #89 (PR-A2), #90 (PR-A1.5), #92 (PR-A3), (PR-A4a opening) | Cost-closure additions (PR-A2 lift, PR-A1.5 cleanup, PR-A3 cache-savings, PR-A4a drift report new code, PR-A4b/c next) | `extracted_llm_infrastructure/services/cost/`, `extracted_llm_infrastructure/storage/migrations/259_*` |
...
| (PR-A4a, opening) | Add drift report (NEW CODE) | `extracted_llm_infrastructure/{manifest.json, services/cost/__init__.py, services/cost/drift.py, README.md, STATUS.md}`; `tests/test_extracted_llm_infrastructure_drift.py`; `docs/extraction/COORDINATION.md` | claude-2026-05-03-b | `services/cost/drift.py` or related test/docs |
...
| PR-A4a | `extracted_llm_infrastructure` | claude-2026-05-03-b | PR-A2, PR-A3 | New code: drift report (local vs invoiced). **In flight.** |
| PR-A4b | `extracted_llm_infrastructure` | unclaimed | PR-A2 | New code: budget gate (daily + per-attribution caps). |
| PR-A4c | `extracted_llm_infrastructure` | unclaimed | none | New code: OpenAI provider adapter (Costs API). |
