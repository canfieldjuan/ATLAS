...
| `extracted_content_pipeline` | 1 -> 2 (productization seams) | #181 | — | Webhook API router seam landed; continue remaining host-facing API seams (`api/b2b_campaigns.py`, `api/seller_campaigns.py`) after DB-backed review/export/send/progression/analytics/webhook paths | none |
