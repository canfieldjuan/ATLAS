...
Last updated: 2026-05-11T00:26Z by codex-content-ops-reasoning-provider-status
...
| (in flight) | Surface Content Ops reasoning provider status in the control-surface catalog | NEW: `plans/PR-Content-Ops-Reasoning-Provider-Status.md`. EDIT: `extracted_content_pipeline/api/control_surfaces.py` (+ optional reasoning status provider); `atlas_brain/_content_ops_reasoning.py` (+ DB/file/none status descriptor); `atlas_brain/api/__init__.py` (wire status provider); `tests/test_extracted_content_control_surface_api.py`; `tests/test_atlas_content_ops_reasoning.py`; `docs/frontend/content_ops_frontend_contract.md`; `docs/extraction/coordination/inflight.md`. | codex-content-ops-reasoning-provider-status | `extracted_competitive_intelligence/manifest.json`; `extracted_competitive_intelligence/autonomous/visibility.py`; `extracted_competitive_intelligence/storage/migrations/246_pipeline_visibility.sql` |
