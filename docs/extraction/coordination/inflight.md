...
|---|---|---|---|---|
| (in flight) | Add email campaign reasoning usage count to Content Ops execution summary | EDIT: `extracted_content_pipeline/campaign_generation.py` (result counter + successful draft count), `tests/test_extracted_campaign_generation.py` (counter assertions), `atlas-intel-ui/src/pages/ContentOpsNewRun.tsx` (summary display), `atlas-intel-ui/src/api/__fixtures__/contentOps/execution-completed.json`, `docs/frontend/content_ops_frontend_contract.md`; NEW: `plans/PR-Content-Ops-Email-Reasoning-Usage-Summary.md`. | codex-content-ops-email-reasoning-usage | Avoid concurrent edits to Content Ops campaign generation result shape and execution result UI summary. |
