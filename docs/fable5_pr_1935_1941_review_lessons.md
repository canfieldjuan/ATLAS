...
| 2 | Every automated-review finding reconciled before merge | **ENFORCED** (pre-dated arc) | `scripts/check_ai_reconciliation_live.py` (live) + `scripts/audit_ai_reconciliation.py` (body shape) |
...
| 4 | Producer/consumer shape fidelity: consumer tests must use fixtures the real producer emits (S6 mass-deletion P1: bare ids vs Reddit API fullnames stayed green) | **OPEN -- codification slice 2** | Planned: shared fixture factory derived from the real producer, so a test physically cannot seed a shape the pipeline never writes |
| 5 | Adversarial-negative fixtures for every boundary (exact bool/int/finite-float, hostile strings) -- reviewers caught 8/8 injection holes in S3 alone | **OPEN -- deferred** | Candidate: negatives-presence check wired into pre-push-audit |
...
| 7 | Gate scripts execute from the PR merge ref (self-passing edit risk) -- applies to EVERY gate above | **TRACKED** | #1944 reconciliation waiver 18; needs one repo-wide trusted-base workflow slice |
