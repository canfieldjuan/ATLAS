...
| 5 | Adversarial-negative fixtures for every boundary (exact bool/int/finite-float, hostile strings) -- reviewers caught 8/8 injection holes in S3 alone | **ENFORCED** | `NO_RAISES_TESTS` + `NO_TEST_FILE` in `SENSITIVE_ZERO_TOLERANCE` (`scripts/maturity_sweep.py`): a sensitive-glob module whose tests never assert a raise (AST-verified, not text-matched) -- or that has no tests at all -- fails the ratchet outright, min-score cannot save it |
