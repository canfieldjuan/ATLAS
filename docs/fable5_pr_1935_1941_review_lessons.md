...
| 4 | Producer/consumer shape fidelity: consumer tests must use fixtures the real producer emits (S6 mass-deletion P1: bare ids vs Reddit API fullnames stayed green) | **ENFORCED** | `tests/atlas_reddit_fixtures.py` factory + `tests/test_atlas_reddit_fixture_fidelity.py` (#1947): seeding runs through the REAL producer, so a test physically cannot seed a shape the pipeline never writes |
| 5 | Adversarial-negative fixtures for every boundary (exact bool/int/finite-float, hostile strings) -- reviewers caught 8/8 injection holes in S3 alone | **ENFORCED** | `NO_RAISES_TESTS` in `SENSITIVE_ZERO_TOLERANCE` (`scripts/maturity_sweep.py`): a sensitive-glob module whose tests never assert a raise fails the ratchet outright, min-score cannot save it |
...
| 7 | Gate scripts execute from the PR merge ref (self-passing edit risk) -- applies to EVERY gate above | **ENFORCED** | Trusted-base gate execution: meta-gates restore gate scripts from the base ref before running (#1949, extended to the pre-push audit by #1950); closed #1944 waiver 18 |
