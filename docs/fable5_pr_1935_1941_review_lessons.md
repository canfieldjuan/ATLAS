...
| 4 | Producer/consumer shape fidelity: consumer tests must use fixtures the real producer emits (S6 mass-deletion P1: bare ids vs fullnames stayed green) | **OPEN -- codification slice 2** | Planned: shared fixture factory derived from the real producer, so a test physically cannot seed a shape the pipeline never writes |
