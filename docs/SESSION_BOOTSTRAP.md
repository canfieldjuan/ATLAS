...
>
> 5. **Plan first** (`plans/PR-<Slice>.md`, the 7 top-level sections plus nested Problem-derived and Review Contracts, <400 LOC soft cap), open PRs ready-for-review (not draft), and run the per-package validation gauntlet before pushing (see CLAUDE.md "Per-package validation gauntlets"). A non-empty Markdown-only human diff may omit the plan only with a PR body beginning `Docs-only: true`; every other human diff adds exactly one plan.
