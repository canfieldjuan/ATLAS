...
>    - **PR-prep helpers — use these; don't hand-format the plan shape or push raw:** `scripts/new_pr_plan.sh <Slice> --lane <lane> --phase "<phase>"` scaffolds the 7-section `plans/PR-<Slice>.md` → implement → `python scripts/sync_pr_plan.py plans/PR-<Slice>.md` rewrites `### Files touched` + the diff-size table from the real diff → `bash scripts/push_pr.sh <pr-body-file> -u origin HEAD` runs `local_pr_review.sh` and pushes with `ATLAS_CURRENT_PR_BODY_FILE` exported so the pre-push hook validates the same body (the most common failed-push cause). Reconstructing the plan shape by hand or pushing without the body env is what burns the formatting/failed-push loop. See AGENTS.md §3a.2.
>
