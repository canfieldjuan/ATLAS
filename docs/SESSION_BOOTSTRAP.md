...
>    - **CI enrollment, same PR (frontend):** UI workflows run explicit per-test lists, not globs. Adding a `test:*` script to a `*-ui/package.json` does NOT make CI run it — add the matching `run: npm run test:<name>` step to that UI workflow in the SAME PR. `scripts/audit_ui_test_enrollment.py` runs in the local/pre-push audit and fails when a declared UI test is not enrolled; do not treat this as manual memory work.
