...
| SYS-017 | Medium | B2B product/plan gate split | Tenant routes are protected from anonymous access, but many use `require_auth` rather than `require_b2b_plan`. First product-surface slice closed: competitive-set list/create/update/delete/preview/run now require `b2b_growth` before repo/DB/service touch. Frontend `usePlanGate` hides some UI, but direct API access still needs endpoint-by-endpoint backend enforcement. | `atlas_brain/api/b2b_tenant_dashboard.py:1549`, `atlas_brain/api/b2b_tenant_dashboard.py:1567`, `atlas_brain/api/b2b_tenant_dashboard.py:1603`, `atlas_brain/api/b2b_tenant_dashboard.py:1656`, `atlas_brain/api/b2b_tenant_dashboard.py:1670`, `atlas_brain/api/b2b_tenant_dashboard.py:1694`, `tests/test_b2b_access_boundaries.py:199`, `tests/test_b2b_access_boundaries.py:277` | Continue classifying explicit tenant routes by required tier. Next likely slices: tracked-vendor mutations, watchlist/report-subscription controls, report generation, and CRM push. Keep `require_auth` only for account/profile routes that are intentionally universal. |
