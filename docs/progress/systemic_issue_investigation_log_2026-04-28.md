...
| SYS-003 | High | CRM event ingestion and visibility | CRM event list and enrichment-stats are now browser/operator APIs: they require auth, scope every query by `account_id`, and are pinned in the access-boundary harness before DB touch. Generic single/batch ingestion still accepts unauthenticated writes with `account_id=NULL`; native webhook endpoints still use session-style optional auth plus a manual user check. | `atlas_brain/api/b2b_crm_events.py:128`, `atlas_brain/api/b2b_crm_events.py:258`, `atlas_brain/api/b2b_crm_events.py:861`, `atlas_brain/api/b2b_crm_events.py:982`, `tests/test_b2b_access_boundaries.py:66`, `tests/test_b2b_crm_events.py:285` | Remaining work is webhook identity, not read visibility: split generic/native ingestion behind signed provider secrets or account-scoped API keys, then remove account-null writes. |
