...
| SYS-006 | High | Admin cost/system API | Admin cost/system telemetry now has router-level `require_auth`, and the access-boundary harness pins unauthenticated `/admin/costs/summary` rejection before DB touch. | `atlas_brain/api/admin_costs.py:38`, `atlas_brain/api/admin_costs.py:41`, `atlas_brain/api/admin_costs.py:663`, `tests/test_b2b_access_boundaries.py:34` | Decide whether authenticated-user access is enough or whether these routes need an admin-only role/internal-network boundary. Keep scraping-summary fixture failures tracked outside this non-scraping audit. |
