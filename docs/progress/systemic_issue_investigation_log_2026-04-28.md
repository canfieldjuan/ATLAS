...
| --- | --- | --- | --- | --- | --- |
| SYS-001 | High | B2B dashboard app auth | `optional_auth` makes unauthenticated users unscoped instead of denied on many app/report endpoints. | `atlas_brain/auth/dependencies.py:123`, `atlas_brain/api/b2b_dashboard.py:745`, `atlas_brain/api/b2b_dashboard.py:2443`, `atlas_brain/api/b2b_dashboard.py:2537` | Classify each `/b2b/dashboard` route as public/demo vs tenant app. Tenant app routes should use `require_b2b_plan`; public routes should move to an explicit public router. |
| SYS-002 | High | Vendor target management | Unauthenticated calls can create global targets, update by id without scope, delete by id, and generate reports. This is not a single bug; the CRUD model still assumes global admin semantics. | `atlas_brain/api/vendor_targets.py:242`, `atlas_brain/api/vendor_targets.py:400`, `atlas_brain/api/vendor_targets.py:568`, `atlas_brain/api/vendor_targets.py:626` | Decide whether vendor targets are tenant-owned app state. If yes, require auth/plan on CRUD and only allow explicit `legacy_global` reads/claims through safe endpoints. |
| SYS-003 | High | CRM event ingestion and visibility | Generic CRM event ingestion accepts unauthenticated writes with `account_id=NULL`; event listing/stats also become global when unauthenticated. Native webhook endpoints require a user after optional auth, but generic endpoints do not. | `atlas_brain/api/b2b_crm_events.py:125`, `atlas_brain/api/b2b_crm_events.py:257`, `atlas_brain/api/b2b_crm_events.py:860`, `atlas_brain/api/b2b_crm_events.py:986` | Split webhook ingestion from operator query APIs. Webhooks need signed provider secrets or API keys; list/stats need auth and account scope. |
...
| SYS-021 | High | Campaign management access boundary | Suppression mutations and sequence controls lived in `b2b_campaigns.py` without an auth dependency, so SaaS auth enabled did not protect global suppression writes or sequence pause/resume/recipient changes. | `atlas_brain/api/b2b_campaigns.py:1205`, `atlas_brain/api/b2b_campaigns.py:1233`, `atlas_brain/api/b2b_campaigns.py:1854`, `atlas_brain/api/b2b_campaigns.py:1887`, `atlas_brain/api/b2b_campaigns.py:1913` | Mutating endpoints now require `b2b_growth` and are pinned in the access-boundary harness. Shared-state writes (the `campaign_suppressions` table has no tenant column — it is a global blocklist by design) and the read endpoints (analytics, sequences, review-queue, audit-log, suppressions list/check) both still need scope authorization, not just authentication — tracked as SYS-017 territory. |
| SYS-005 | High | Vendor briefing public/admin endpoints | Preview/generate/send-batch/list are optional-auth while gate/checkout/report-data are public by design. Internal send/list controls are mixed with public sales-flow endpoints in one router. | `atlas_brain/api/b2b_vendor_briefing.py:255`, `atlas_brain/api/b2b_vendor_briefing.py:279`, `atlas_brain/api/b2b_vendor_briefing.py:702`, `atlas_brain/api/b2b_vendor_briefing.py:716` | Split public briefing flow from operator briefing controls. Public flow should be token/payment-gated; operator preview/generate/send/list should require auth/plan. |
| SYS-006 | High | Admin cost/system API | Admin cost, recent LLM calls, run details, task health, error timeline, and system resources are mounted under `/api/v1/admin/costs` with no auth dependency. | `atlas_brain/api/admin_costs.py:37`, `atlas_brain/api/admin_costs.py:658`, `atlas_brain/api/admin_costs.py:2532`, `atlas_brain/api/admin_costs.py:3102`, `atlas_brain/api/admin_costs.py:4851` | Confirm deployment exposure. If reachable outside localhost/admin proxy, add admin auth dependency or move behind an internal-only router. |
| SYS-007 | High | Amazon Seller campaign API | Seller target CRUD, campaign generation, campaign listing, intelligence listing, and refresh are all unauthenticated while mounted under `/api/v1`. | `atlas_brain/api/seller_campaigns.py:128`, `atlas_brain/api/seller_campaigns.py:187`, `atlas_brain/api/seller_campaigns.py:295`, `atlas_brain/api/seller_campaigns.py:314`, `atlas_brain/api/seller_campaigns.py:446` | Decide whether this is internal tooling or product UI. If product/internal admin, require auth; if public, carve out only read-safe public endpoints. |
...
| SYS-009 | High | Generic report renderers | Generic renderer dangerous sections now fail closed for top-level cross-vendor battles, objection handlers, recommended plays, talk tracks, and weakness analysis unless every rendered item carries report-safe ProductClaim context. Legacy/unvalidated sections show a gate fallback instead of winner/play/recommendation language. | `atlas-churn-ui/src/components/report-renderers/StructuredReportData.tsx:61`, `atlas-churn-ui/src/components/report-renderers/StructuredReportData.tsx:635`, `atlas-churn-ui/src/components/report-renderers/StructuredReportData.tsx:797`, `atlas-churn-ui/src/components/report-renderers/StructuredReportData.test.tsx:123` | Continue with specialized-renderer coverage in SYS-018 and any future generic dangerous keys discovered in report payloads. Defense-in-depth follow-up: nested dangerous keys routed through `MixedObjectCard` still need an inner `StructuredReportValue` gate if real payloads use nested shapes. |
| SYS-018 | High | Specialized report renderer residuals | Patch 6 gated head-to-head and battle-card displacement reasoning. Battle-card `cross_vendor_battles`, `talk_track`, `recommended_plays`, and category-council winner language, plus weekly-feed `top_displacement_targets`, now fail closed unless each row/block carries a report-safe ProductClaim. Remaining specialized residuals still include market winners from legacy displacement-report rows. | `atlas-churn-ui/src/components/report-renderers/SpecializedReportData.tsx:896`, `atlas-churn-ui/src/components/report-renderers/SpecializedReportData.tsx:1303`, `atlas-churn-ui/src/components/report-renderers/SpecializedReportData.tsx:1392`, `atlas-churn-ui/src/components/report-renderers/SpecializedReportData.tsx:1876`, `atlas-churn-ui/src/components/report-renderers/SpecializedReportData.tsx:2532` | Continue the report-claim coverage matrix by view model. Any remaining report section with winner/play/recommendation/displacement semantics should require ProductClaim gate context or render as legacy/unvalidated. |
| SYS-010 | Medium | Vendor Workspace claim coverage | DM churn and price complaint cards are gated, but NPS proxy, churn intent count, slow-burn metrics, and reasoning narrative still render raw signal fields. | `atlas-churn-ui/src/pages/VendorDetail.tsx:1200`, `atlas-churn-ui/src/pages/VendorDetail.tsx:1234`, `atlas-churn-ui/src/pages/VendorDetail.tsx:1352`, `atlas_brain/api/b2b_vendor_claims.py:189` | Add a Vendor Workspace contract-coverage pass. Either gate more cards or visibly label uncontracted fields as legacy/unvalidated. |
| SYS-011 | Medium | ProductClaim policy registry | Unregistered claim types fall back to default thresholds, which is convenient but weakens the closed-contract model for future customer-facing claims. | `atlas_brain/services/b2b/product_claim.py:95`, `atlas_brain/services/b2b/product_claim.py:125`, `atlas_brain/services/b2b/product_claim.py:428` | Consider strict policy registration for production aggregators, with tests that new claim types cannot publish under implicit defaults. |
...
| SYS-013 | Medium | Public report-data package | Public briefing `report-data` correctly uses a gate token, but it still packages cached briefing data, intelligence reports, and product profiles without a ProductClaim coverage boundary. | `atlas_brain/api/b2b_vendor_briefing.py:588`, `atlas_brain/api/b2b_vendor_briefing.py:631`, `atlas_brain/api/b2b_vendor_briefing.py:670` | Audit exactly what public report-data exposes after redaction. Apply ProductClaim-aware packaging for dangerous report sections before expanding public access. |
| SYS-014 | Low/Medium | Evidence lazy-on-read fallback | Evidence detail GET can compute and persist grounding status. This is documented as a migration fallback, but if hit count rises it indicates write-path/backfill drift. | `atlas_brain/api/b2b_evidence.py:382`, `atlas_brain/api/b2b_evidence.py:397`, `atlas_brain/api/b2b_evidence.py:427` | Add/verify an operational counter or audit query for `lazy_grounding_fallback_hit`; steady-state should be zero. |
| SYS-015 | Low/Medium | ACCOUNT source-review count coupling | `source_review_count` is serialized from `claim.sample_size`; this works only while sample size means source review count. | `atlas_brain/services/b2b/account_opportunity_claims.py:97`, `atlas_brain/services/b2b/account_opportunity_claims.py:149` | Pass `source_review_count` explicitly or introduce an ACCOUNT-specific serialized wrapper before denominator semantics change. |
| SYS-016 | Medium | API prefix fallback | The frontend retries `/api/v1/...` requests against `/api/...` on 404. That helps during migrations, but it can mask router registration/prefix drift and route-level auth differences. | `atlas-churn-ui/src/api/client.ts:82`, `atlas-churn-ui/src/api/client.ts:88`, `atlas-churn-ui/src/auth/AuthContext.tsx:11` | Inventory which fallback paths are still needed. Remove global fallback or restrict it to a small allowlist with tests for production-effective mount paths. |
| SYS-017 | Medium | B2B product/plan gate split | Tenant routes are protected from anonymous access, but many use `require_auth` rather than `require_b2b_plan`. Frontend `usePlanGate` hides some UI, but direct API access may not enforce B2B product/tier consistently. | `atlas_brain/api/b2b_tenant_dashboard.py:1177`, `atlas_brain/api/b2b_tenant_dashboard.py:2001`, `atlas_brain/api/b2b_tenant_dashboard.py:3965`, `atlas-churn-ui/src/hooks/usePlanGate.ts:8` | Classify tenant endpoints by required tier. Convert product surfaces to `require_b2b_plan(...)`; keep `require_auth` only for account/profile routes that are intentionally universal. |
| SYS-019 | Medium | Autonomous competitive-set liveness | `b2b_reasoning_synthesis` can execute successfully while doing no scheduled competitive-set work because every set is filtered out by domain-specific eligibility. A stuck `last_run_status='running'` can also remove a set from due selection even though scheduler orphan cleanup only fixes `task_executions`. | `atlas_brain/storage/repositories/competitive_set.py:299`, `atlas_brain/storage/repositories/competitive_set.py:310`, `atlas_brain/storage/repositories/competitive_set.py:313`, `atlas_brain/autonomous/tasks/b2b_reasoning_synthesis.py:1258`, `atlas_brain/autonomous/tasks/b2b_reasoning_synthesis.py:1262`, `atlas_brain/autonomous/scheduler.py:162` | Add a liveness audit for competitive-set due queues: counts by ineligibility reason, stale-running reset policy, and alert when scheduled task executions repeatedly produce `No due competitive sets`. |
| SYS-020 | Medium | Safety envelope schema contracts | ProductClaim claim endpoints use strict Pydantic response models, but evidence and opportunity rows still depend on plain dict shaping and optional frontend fields. Missing gate fields have already produced fail-open classes; the contract should be enforced at API boundaries, not only in React. | `atlas_brain/api/b2b_vendor_claims.py:82`, `atlas_brain/api/b2b_vendor_claims.py:170`, `atlas_brain/api/b2b_challenger_claims.py:114`, `atlas_brain/api/b2b_evidence.py:167`, `atlas_brain/api/b2b_evidence.py:338`, `atlas-churn-ui/src/types/index.ts:133` | Promote safety-bearing response shapes to Pydantic models and make frontend gate fields non-optional inside those models. Keep rollout optionality only at the row wrapper level with explicit legacy/fail-closed semantics. |
