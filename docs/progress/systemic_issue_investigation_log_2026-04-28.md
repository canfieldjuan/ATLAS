...
| --- | --- | --- | --- | --- | --- |
| SYS-001 | High | B2B dashboard app auth | Legacy `/api/v1/b2b/dashboard` paths are hard-removed by middleware, but unmigrated legacy endpoints are still registered as `/api/v1/b2b/tenant` aliases. Company-signal approve/suppress candidate and group mutations now require `b2b_growth` and are pinned before DB touch; remaining aliased dashboard routes still need public/demo vs tenant-app classification. | `atlas_brain/main.py:740`, `atlas_brain/api/b2b_tenant_dashboard.py:4818`, `atlas_brain/api/b2b_dashboard.py:3990`, `atlas_brain/api/b2b_dashboard.py:4720`, `tests/test_b2b_access_boundaries.py:93` | Continue classifying tenant-alias dashboard routes. Tenant app reads/writes should use `require_b2b_plan`; any intentionally public/demo routes should move to an explicit public router rather than inheriting legacy dependencies. |
