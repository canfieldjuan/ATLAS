...
Usage:
    python scripts/test_adapter_live.py [--base-url http://127.0.0.1:8000]
"""
...
def test_high_intent(base: str) -> list[str]:
    """Phase 1: list_high_intent via dashboard API."""
    print("\n--- Phase 1: High Intent Companies ---")
    url = f"{base}/api/v1/b2b/dashboard/high-intent?min_urgency=7&window_days=90&limit=10"
    data = _get(url)
...
def test_high_intent_vendor_scoped(base: str) -> list[str]:
    """Phase 1: vendor-scoped high intent."""
    print("\n--- Phase 1: High Intent (vendor=HubSpot) ---")
    url = f"{base}/api/v1/b2b/dashboard/high-intent?vendor_name=HubSpot&min_urgency=5&window_days=90&limit=5"
    data = _get(url)
    checks = [
...
def test_search_reviews(base: str) -> list[str]:
    """Phase 2: search_reviews via dashboard API."""
    print("\n--- Phase 2: Search Reviews ---")
    url = f"{base}/api/v1/b2b/dashboard/reviews?window_days=90&limit=10"
    data = _get(url)
...
def test_search_reviews_filtered(base: str) -> list[str]:
    """Phase 2: search with pain_category filter."""
    print("\n--- Phase 2: Search Reviews (pain=pricing) ---")
    url = f"{base}/api/v1/b2b/dashboard/reviews?pain_category=pricing&window_days=90&limit=5"
    data = _get(url)
...
def test_search_reviews_churn_intent(base: str) -> list[str]:
    """Phase 2: search with churn intent filter."""
    print("\n--- Phase 2: Search Reviews (churn_intent=true) ---")
    url = f"{base}/api/v1/b2b/dashboard/reviews?has_churn_intent=true&window_days=90&limit=5"
    data = _get(url)
    checks = [("response has reviews", "reviews" in data)]
    if data.get("reviews"):
        checks.append((
...
def test_vendor_profile(base: str) -> list[str]:
    """Phase 1: get_vendor_profile high-intent sub-query."""
    print("\n--- Phase 1: Vendor Profile (HubSpot) ---")
    url = f"{base}/api/v1/b2b/dashboard/vendors/HubSpot"
    data = _get(url)
    checks = [
...
def test_null_semantics(base: str) -> list[str]:
    """Phase 2: verify null urgency/intent stays null, not 0/false."""
    print("\n--- Null Semantics Check ---")
    url = f"{base}/api/v1/b2b/dashboard/reviews?window_days=90&limit=50"
    data = _get(url)
    checks = [("response has reviews", "reviews" in data)]
...
def test_suppression_impact(base: str) -> list[str]:
    """Check if suppression filtering is active."""
    print("\n--- Suppression Impact ---")
    # Query with a wide window to see if suppressed reviews are filtered
    url = f"{base}/api/v1/b2b/dashboard/reviews?window_days=365&limit=100"
    data = _get(url)
...
def main():
    ...
    all_failures = []
    all_failures.extend(test_high_intent(base))
    all_failures.extend(test_high_intent_vendor_scoped(base))
    all_failures.extend(test_vendor_profile(base))
    all_failures.extend(test_search_reviews(base))
    all_failures.extend(test_search_reviews_filtered(base))
    all_failures.extend(test_search_reviews_churn_intent(base))
    all_failures.extend(test_null_semantics(base))
    all_failures.extend(test_suppression_impact(base))
