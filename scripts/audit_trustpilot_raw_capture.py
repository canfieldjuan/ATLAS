#!/usr/bin/env python3
"""Capture raw Trustpilot pages and audit employer/title field presence."""
...
from atlas_brain.services.scraping.trustpilot_audit import (
    capture_trustpilot_page,
    write_trustpilot_capture_artifacts,
)
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--vendor-name", help="Resolve a Trustpilot target by vendor name")
    parser.add_argument("--product-slug", help="Resolve a Trustpilot target by product_slug")
    ...
    parser.add_argument(
        "--output-dir",
        default=str(ROOT / "data" / "audits" / "trustpilot_raw"),
...
async def _resolve_target(args: argparse.Namespace) -> ScrapeTarget:
    ...
    if not row:
        raise RuntimeError("No matching Trustpilot target found")
...
def _selected_methods(method: str) -> list[str]:
    ...
    if str(settings.b2b_scrape.web_unlocker_url or "").strip() and "trustpilot.com" in unlocker_domains:
...
async def _run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    for page in range(1, max(1, args.pages) + 1):
        for method in methods:
            try:
                capture = await capture_trustpilot_page(target, page=page, method=method)
                ...
                if args.method == "auto":
                    continue
                continue
            artifact_paths = write_trustpilot_capture_artifacts(
                output_dir,
