#!/usr/bin/env python3
"""Audit Gartner targets for page availability and reviewer-identity field presence."""
...
from atlas_brain.services.scraping.gartner_audit import capture_gartner_page
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--limit", type=int, default=10, help="How many enabled Gartner targets to audit")
...
async def _load_targets(args: argparse.Namespace) -> list[ScrapeTarget]:
    pool = get_db_pool()
    filters = ["source = 'gartner'", "enabled = true"]
    params: list[Any] = []
    idx = 1
    if args.vendor_name:
...
async def _run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    for target in targets:
        try:
            capture = await capture_gartner_page(target, page=1, method=args.method)
            classification = _classify_capture_result(capture)
            analysis = capture.get("analysis") or {}
            next_data = analysis.get("next_data") or {}
            ...
            results.append(
                {
                    ...
                    "next_data_review_count": next_data.get("review_object_count"),
                    "next_data_company_count": next_data.get("reviewer_company_count"),
