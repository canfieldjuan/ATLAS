#!/usr/bin/env python3
"""Audit Capterra targets for page availability and reviewer-identity field presence."""
...
from atlas_brain.services.scraping.capterra_audit import capture_capterra_page
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--limit", type=int, default=10, help="How many enabled Capterra targets to audit")
...
async def _load_targets(args: argparse.Namespace) -> list[ScrapeTarget]:
    pool = get_db_pool()
    filters = ["source = 'capterra'", "enabled = true"]
    params: list[Any] = []
    idx = 1
    if args.vendor_name:
...
async def _run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    for target in targets:
        try:
            capture = await capture_capterra_page(target, page=1, method=args.method)
