#!/usr/bin/env python3
"""Audit Trustpilot targets for page availability and reviewer-identity field presence.

This is intentionally read-only. It captures page 1 for a bounded set of enabled
Trustpilot targets and reports:
- HTTP status
- whether review-level employer fields exist
- whether review-level title fields exist
- whether the page is field-empty for named-account use

Usage:
  python scripts/audit_trustpilot_target_hygiene.py
  python scripts/audit_trustpilot_target_hygiene.py --limit 10
  python scripts/audit_trustpilot_target_hygiene.py --vendor-name Jira
"""
...
from atlas_brain.services.scraping.trustpilot_audit import capture_trustpilot_page
from atlas_brain.storage.database import close_database, get_db_pool, init_database


def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--limit", type=int, default=10, help="How many enabled Trustpilot targets to audit")
    parser.add_argument("--vendor-name", help="Optional exact vendor-name filter")
    parser.add_argument(
        "--method",
        choices=("web_unlocker", "http"),
...
def _classify_capture_result(capture: dict[str, Any]) -> str:
    ...
    if status_code != 200:
        return "fetch_error"
    if bool(analysis.get("employer_fields_present")) or bool(analysis.get("title_fields_present")):
        return "reviewer_identity_present"
    return "field_empty_for_named_account"


async def _load_targets(args: argparse.Namespace) -> list[ScrapeTarget]:
    pool = get_db_pool()
    filters = ["source = 'trustpilot'", "enabled = true"]
    params: list[Any] = []
    idx = 1
    if args.vendor_name:
...
async def _run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    for target in targets:
        try:
            capture = await capture_trustpilot_page(target, page=1, method=args.method)
            ...
            results.append(
                {
                    ...
                    "review_object_count": (analysis.get("jsonld") or {}).get("review_object_count"),
                    "review_card_count": (analysis.get("markup") or {}).get("review_card_count"),
                }
            )
    ...
    summary = {
        ...
        "identity_present_count": sum(
            1 for item in results if item.get("classification") == "reviewer_identity_present"
        ),
