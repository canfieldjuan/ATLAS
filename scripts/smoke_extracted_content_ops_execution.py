...
def _fake_consumed_reasoning_contexts(count: int) -> list[dict[str, Any]]:
    return [
        {
...
class _ReasoningAwareOpportunityAssetService(_OpportunityAssetService):
    ...
    async def generate(self, **kwargs: Any) -> dict[str, Any]:
        ...
        if self.provider is not None:
            ...
            result["consumed_reasoning_contexts"] = _fake_consumed_reasoning_contexts(
                count
            )
        return result
...
def _reasoning_usage_errors(
    ...
    if isinstance(reasoning, Mapping) and isinstance(result_count, int):
