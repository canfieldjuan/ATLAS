#!/usr/bin/env python3
"""Capture raw Capterra pages and audit reviewer field presence."""
...
from atlas_brain.services.scraping.capterra_audit import (
    ...
    write_capterra_capture_artifacts,
)
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--vendor-name", help="Resolve a Capterra target by vendor name")
    parser.add_argument("--product-slug", help="Resolve a Capterra target by product_slug")
    parser.add_argument("--pages", type=int, default=1, help="How many pages to capture")
    parser.add_argument(
        "--method",
        choices=("auto", "http", "web_unlocker", "browser", "both"),
        default="auto",
        help="Capture path to use",
    )
    parser.add_argument(
        "--output-dir",
        default=str(ROOT / "data" / "audits" / "capterra_raw"),
...
async def _resolve_target(args: argparse.Namespace) -> ScrapeTarget:
    ...
    if not row:
        raise RuntimeError("No matching Capterra target found")
...
def _selected_methods(method: str) -> list[str]:
    ...
    if str(settings.b2b_scrape.web_unlocker_url or "").strip() and "capterra.com" in unlocker_domains:
...
async def _run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    for page in range(1, max(1, args.pages) + 1):
        for method in methods:
            try:
                capture = await capture_capterra_page(target, page=page, method=method)
            except Exception as exc:
                captures.append({"page": page, "method": method, "error": str(exc)})
                if args.method == "auto":
                    continue
                continue
            artifact_paths = write_capterra_capture_artifacts(
                output_dir,
