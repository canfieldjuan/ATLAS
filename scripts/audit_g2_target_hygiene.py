#!/usr/bin/env python3
"""Audit G2 targets for page availability and reviewer-identity field presence."""
...
from atlas_brain.services.scraping.g2_audit import capture_g2_page
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--limit", type=int, default=10, help="How many enabled G2 targets to audit")
...
async def _load_targets(args: argparse.Namespace) -> list[ScrapeTarget]:
    pool = get_db_pool()
    filters = ["source = 'g2'", "enabled = true"]
    params: list[Any] = []
    idx = 1
    if args.vendor_name:
...
async def _run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    for target in targets:
        try:
            capture = await capture_g2_page(target, page=1, method=args.method)
