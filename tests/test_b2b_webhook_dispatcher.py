...
async def test_validate_webhook_destination_blocks_public_hostname_resolving_to_non_global_address():
    with patch.object(
        ...
        AsyncMock(return_value={"127.0.0.1", "198.51.100.10"}),
    ):
        ok, error = await webhook_dispatcher.validate_webhook_destination(
...
async def test_deliver_single_logs_blocked_destination_without_http_request():
    ...
    assert args[14] == "Webhook destination cannot target non-global address: 127.0.0.1"
...
async def test_send_test_webhook_returns_destination_error_when_blocked():
    ...
    with patch("atlas_brain.config.settings", SimpleNamespace(b2b_webhook=cfg)):
...
@pytest.mark.asyncio
async def test_send_test_webhook_returns_http_failure_reason():
    pool = MagicMock()
    pool.fetchrow = AsyncMock(
        return_value={
            "id": "sub-1",
    ...
    client = MagicMock()
    client.post = AsyncMock(return_value=SimpleNamespace(status_code=502, text="upstream rejected"))
    ...
    with patch("atlas_brain.config.settings", SimpleNamespace(b2b_webhook=cfg)):
        with patch.object(
            ...
            with patch("atlas_brain.services.b2b.webhook_dispatcher.httpx.AsyncClient", return_value=client_ctx):
                result = await webhook_dispatcher.send_test_webhook(pool, "sub-1")
    ...
    assert result == {
        ...
        "subscription_id": "sub-1",
        "channel": "generic",
        "error": "HTTP 502: upstream rejected",
    }
    args = pool.execute.await_args.args
    assert args[2] == "test"
    assert args[9] == 502
    assert args[10] == "upstream rejected"
    assert args[14] is None


@pytest.mark.asyncio
async def test_send_test_webhook_returns_timeout_reason():
    pool = MagicMock()
    pool.fetchrow = AsyncMock(
        return_value={
            "id": "sub-1",
    ...
    client = MagicMock()
    client.post = AsyncMock(side_effect=webhook_dispatcher.httpx.TimeoutException("timed out"))
    client_ctx = MagicMock()
    client_ctx.__aenter__ = AsyncMock(return_value=client)
    client_ctx.__aexit__ = AsyncMock(return_value=None)
