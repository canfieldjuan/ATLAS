...
# ---------------------------------------------------------------------------
# Pre-scrape low-incremental-yield gate (no migration; uses existing columns)
# ---------------------------------------------------------------------------


def _low_yield_cfg(**overrides):
    base = {
        # Cross-source knobs (helper imports them but never reads them here)
        "pre_scrape_skip_paid_sources_override": "",
        # Low-yield knobs
        "pre_scrape_low_yield_skip_enabled": True,
        "pre_scrape_low_yield_skip_lookback_runs": 5,
        "pre_scrape_low_yield_skip_ratio_threshold": 0.05,
        "pre_scrape_low_yield_skip_min_found_total": 50,
        "pre_scrape_low_yield_skip_max_age_days": 14,
    }
    base.update(overrides)
    return SimpleNamespace(**base)
...
@pytest.mark.asyncio
async def test_low_yield_skip_returns_none_when_disabled():
    from atlas_brain.autonomous.tasks.b2b_scrape_intake import _evaluate_pre_scrape_low_yield_skip

    pool = _FakeFetchrowPool({
        "real_runs": 99, "total_found": 999, "total_inserted": 0,
        "total_cross_source_duplicates": 0, "total_unique_inserted": 0,
        "last_real_scrape_at": datetime.now(timezone.utc),
    })
    cfg = _low_yield_cfg(pre_scrape_low_yield_skip_enabled=False)

    result = await _evaluate_pre_scrape_low_yield_skip(
        pool, target_id="t-1", source="g2", vendor_name="HubSpot",
        parser_version="g2:test", cfg=cfg,
    )
...
@pytest.mark.asyncio
async def test_low_yield_skip_returns_none_for_non_paid_source():
    from atlas_brain.autonomous.tasks.b2b_scrape_intake import _evaluate_pre_scrape_low_yield_skip
    ...
    result = await _evaluate_pre_scrape_low_yield_skip(
        pool, target_id="t-1", source="reddit", vendor_name="HubSpot",
        parser_version="reddit:test", cfg=_low_yield_cfg(),
    )
...
@pytest.mark.asyncio
async def test_low_yield_skip_returns_none_when_parser_version_missing():
    """Parser-version safety: if the current parser has no version string,
    we cannot safely scope the lookback, so always proceed with the scrape."""
    from atlas_brain.autonomous.tasks.b2b_scrape_intake import _evaluate_pre_scrape_low_yield_skip

    pool = _FakeFetchrowPool({
        "real_runs": 5, "total_found": 200, "total_inserted": 1,
        "total_cross_source_duplicates": 0, "total_unique_inserted": 1,
        "last_real_scrape_at": datetime.now(timezone.utc),
    })

    result = await _evaluate_pre_scrape_low_yield_skip(
        pool, target_id="t-1", source="g2", vendor_name="HubSpot",
        parser_version=None, cfg=_low_yield_cfg(),
    )
...
@pytest.mark.asyncio
async def test_low_yield_skip_query_filters_by_parser_version():
    """The lookback SQL must filter on parser_version so historical
    broken-parser logs cannot suppress a newly fixed parser."""
    from atlas_brain.autonomous.tasks.b2b_scrape_intake import _evaluate_pre_scrape_low_yield_skip

    pool = _FakeFetchrowPool({
        "real_runs": 5, "total_found": 200, "total_inserted": 5,
        "total_cross_source_duplicates": 0, "total_unique_inserted": 5,
        "last_real_scrape_at": datetime.now(timezone.utc),
    })

    await _evaluate_pre_scrape_low_yield_skip(
        pool, target_id="t-1", source="capterra", vendor_name="HubSpot",
        parser_version="capterra:7", cfg=_low_yield_cfg(),
    )

    assert len(pool.fetchrow_calls) == 1
    sql_text, args = pool.fetchrow_calls[0]
    assert "parser_version = $3" in sql_text
    assert "capterra:7" in args


@pytest.mark.asyncio
async def test_low_yield_skip_returns_none_when_real_runs_below_lookback():
    from atlas_brain.autonomous.tasks.b2b_scrape_intake import _evaluate_pre_scrape_low_yield_skip

    pool = _FakeFetchrowPool({
        "real_runs": 3, "total_found": 200, "total_inserted": 1,
        "total_cross_source_duplicates": 0, "total_unique_inserted": 1,
        "last_real_scrape_at": datetime.now(timezone.utc),
    })
...
@pytest.mark.asyncio
async def test_low_yield_skip_returns_none_when_total_found_zero():
    """Defense in depth: zero-found runs alone must never trigger this gate.
    Even if all 5 runs returned 0 found (e.g. transient block recovered as
    'success' with empty result), do NOT skip."""
    from atlas_brain.autonomous.tasks.b2b_scrape_intake import _evaluate_pre_scrape_low_yield_skip

    pool = _FakeFetchrowPool({
        "real_runs": 5, "total_found": 0, "total_inserted": 0,
        "total_cross_source_duplicates": 0, "total_unique_inserted": 0,
        "last_real_scrape_at": datetime.now(timezone.utc),
    })
    ...
    result = await _evaluate_pre_scrape_low_yield_skip(
        pool, target_id="t-1", source="g2", vendor_name="HubSpot",
        parser_version="g2:test", cfg=_low_yield_cfg(),
    )
...
@pytest.mark.asyncio
async def test_low_yield_skip_returns_none_when_total_found_below_min_floor():
    from atlas_brain.autonomous.tasks.b2b_scrape_intake import _evaluate_pre_scrape_low_yield_skip

    pool = _FakeFetchrowPool({
        "real_runs": 5, "total_found": 30, "total_inserted": 0,
        "total_cross_source_duplicates": 0, "total_unique_inserted": 0,
        "last_real_scrape_at": datetime.now(timezone.utc),
    })
...
@pytest.mark.asyncio
async def test_low_yield_skip_uses_unique_insert_ratio_not_raw_inserted():
    """The decision metric must be (inserted - cross_source_duplicates) / found,
    NOT inserted / found. A target whose inserts are mostly cross-source
    duplicates is producing zero net-new value."""
    from atlas_brain.autonomous.tasks.b2b_scrape_intake import _evaluate_pre_scrape_low_yield_skip

    # 200 found, 50 inserted, but 49 of those inserts were cross-source dups
    # so total_unique_inserted = max(50 - 49, 0) = 1; ratio = 1/200 = 0.005
    pool = _FakeFetchrowPool({
        "real_runs": 5, "total_found": 200, "total_inserted": 50,
        "total_cross_source_duplicates": 49, "total_unique_inserted": 1,
        "last_real_scrape_at": datetime.now(timezone.utc),
    })
    ...
    assert result is not None
    assert result["reason"] == "pre_scrape_low_incremental_yield"
    assert result["unique_insert_ratio"] == 0.005
    # Audit fields must keep raw values too
    assert result["total_inserted"] == 50
    assert result["total_cross_source_duplicates"] == 49
    assert result["total_unique_inserted"] == 1
    # SQL must compute the unique inserted column with GREATEST(...)
    sql_text = pool.fetchrow_calls[0][0]
    assert "GREATEST(reviews_inserted - COALESCE(cross_source_duplicates, 0), 0)" in sql_text


@pytest.mark.asyncio
async def test_low_yield_skip_returns_none_when_ratio_above_threshold():
    from atlas_brain.autonomous.tasks.b2b_scrape_intake import _evaluate_pre_scrape_low_yield_skip

    # 200 found, 30 net-new -> ratio 0.15, well above 0.05 threshold
    pool = _FakeFetchrowPool({
        "real_runs": 5, "total_found": 200, "total_inserted": 30,
        "total_cross_source_duplicates": 0, "total_unique_inserted": 30,
        "last_real_scrape_at": datetime.now(timezone.utc),
    })
...
@pytest.mark.asyncio
async def test_low_yield_skip_escape_hatch_uses_last_real_scrape_age():
    from atlas_brain.autonomous.tasks.b2b_scrape_intake import _evaluate_pre_scrape_low_yield_skip
    ...
    result = await _evaluate_pre_scrape_low_yield_skip(
        pool, target_id="t-1", source="capterra", vendor_name="HubSpot",
        parser_version="capterra:7",
        cfg=_low_yield_cfg(pre_scrape_low_yield_skip_max_age_days=14),
    )

    assert result is None  # escape hatch fires


@pytest.mark.asyncio
async def test_log_pre_scrape_skip_writes_low_yield_status_and_stop_reason():
    from atlas_brain.autonomous.tasks.b2b_scrape_intake import _log_pre_scrape_skip
    ...
    assert len(pool.execute_calls) == 1
    sql, args = pool.execute_calls[0]
    assert args[2] == "skipped_low_incremental_yield"  # status
    assert args[8] == "none"                            # proxy_type
    assert args[14] == "pre_scrape_low_incremental_yield"  # stop_reason
    # Status starts with 'skipped' so existing source_yield + admin_costs
    # filters catch it without patching
    assert args[2].startswith("skipped")


def test_manual_trigger_endpoint_does_not_import_low_yield_helper():
    """Regression guard: manual trigger_scrape lives in api/b2b_scrape.py and
    must NOT consult either pre-scrape gate."""
    import inspect
    from atlas_brain.api import b2b_scrape as b2b_scrape_api

    src = inspect.getsource(b2b_scrape_api)
    assert "_evaluate_pre_scrape_low_yield_skip" not in src
