...
def test_manual_trigger_endpoint_imports_eligibility_decision_surface():
    """Phase 1 routes both autonomous and manual paths through
    eligibility.should_scrape_now() / apply_skip_decision(). Assert the
    manual path imports the new decision surface (positive proof of
    migration; replaces the prior 'does not import deleted helper'
    guards which became vacuous after Turn N+3b)."""
    import inspect
    from atlas_brain.api import b2b_scrape as b2b_scrape_api

    src = inspect.getsource(b2b_scrape_api)
    assert "should_scrape_now" in src
    assert "apply_skip_decision" in src
