...
async def test_accounts_in_motion_feed_uses_preview_signal_score_for_account_alerts(monkeypatch):
    ...
    assert result["accounts"][0]["account_alert_eligible"] is True
    assert result["accounts"][0]["account_alert_policy_reason"] is None
    assert result["accounts"][0]["account_reasoning_preview_only"] is True


@pytest.mark.asyncio
async def test_accounts_in_motion_feed_blocks_preview_alert_without_policy_support(monkeypatch):
    from atlas_brain.api import b2b_tenant_dashboard as mod
    ...
    assert result["accounts"][0]["account_alert_score"] == pytest.approx(8.2)
    assert result["accounts"][0]["account_alert_score_source"] == "preview_signal_score"
    assert result["accounts"][0]["account_alert_eligible"] is False
    assert result["accounts"][0]["account_alert_policy_reason"] == "preview_low_confidence"
...
async def test_watchlist_views_list_returns_account_scoped_rows(monkeypatch):
    ...
    assert result["views"][0]["last_alert_delivery_suppressed_preview_summary"] == {
        "count": 1,
        "reasons": {"preview_low_confidence": 1},
    }
