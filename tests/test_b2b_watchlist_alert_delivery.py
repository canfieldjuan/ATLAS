...
async def test_run_processes_due_view_and_advances_schedule(monkeypatch):
    ...
    monkeypatch.setattr(mod.tenant_mod, "_watchlist_alert_sender_configured", lambda: True)
    monkeypatch.setattr(mod.tenant_mod, "get_campaign_sender", lambda: object())
    monkeypatch.setattr(
        mod.tenant_mod,
        "_evaluate_watchlist_alert_events_for_view",
    ...
    monkeypatch.setattr(
        mod.tenant_mod,
        "_resolve_watchlist_alert_recipients",
        AsyncMock(return_value=[{"email": "owner@example.com", "full_name": "Owner"}]),
    )
    monkeypatch.setattr(mod.tenant_mod, "_watchlist_alert_from_email", lambda: "Atlas <atlas@example.com>")
    monkeypatch.setattr(mod.tenant_mod, "_watchlist_manage_url", lambda: "https://app.example.com/watchlists")
    monkeypatch.setattr(
        mod.tenant_mod,
        "_watchlist_alert_event_email_rows",
        lambda events: [{"summary": item["summary"]} for item in events],
    )
    monkeypatch.setattr(
        mod.tenant_mod,
        "_send_watchlist_alert_email",
        AsyncMock(return_value=(True, "msg-1", None, False)),
    )
    monkeypatch.setattr(mod.tenant_mod, "_update_watchlist_alert_email_log", AsyncMock())
    monkeypatch.setattr(mod.tenant_mod, "render_watchlist_alert_delivery_html", lambda **_kwargs: "<p>body</p>")
    ...
    assert mod.tenant_mod._update_watchlist_alert_email_log.await_count == 1
