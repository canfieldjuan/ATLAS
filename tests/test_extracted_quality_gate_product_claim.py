"""Pin the standalone extracted_quality_gate ProductClaim contract.
...
from extracted_quality_gate.ports import QualityPorts
...
def test_public_api_imports_without_atlas_runtime():
    """The standalone package must import through public modules only."""
    ...
    assert QualityPorts().audit_log is None
    assert QualityPolicy(name="default").version == "v1"
    assert QualityInput(artifact_type="blog").claims == ()


def test_quality_report_exposes_terminal_verdict_and_findings():
    blocker = GateFinding(
...
# ----------------------------------------------------------------------------
# Truth-table invariant: report is a subset of render across all combos.
# ----------------------------------------------------------------------------
