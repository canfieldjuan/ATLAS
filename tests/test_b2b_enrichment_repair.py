...
@pytest.mark.asyncio
async def test_run_honors_task_metadata_override_for_orphan_timeout(monkeypatch):
    ...
    monkeypatch.setattr(repair_mod, "_recover_orphaned_repairing", recover)
    ...
    task.metadata = {
        **task.metadata,
        "enrichment_repair_orphan_timeout_minutes": 7,
    }

    await repair_mod.run(task)

    assert recover.await_count == 1
    assert recover.await_args.kwargs["orphan_timeout_minutes"] == 7


@pytest.mark.asyncio
async def test_run_honors_task_metadata_override_for_circuit_breaker(monkeypatch):
    monkeypatch.setattr(repair_mod.settings.b2b_churn, "enabled", True, raising=False)
    monkeypatch.setattr(repair_mod.settings.b2b_churn, "enrichment_repair_enabled", True, raising=False)
    monkeypatch.setattr(repair_mod.settings.b2b_churn, "enrichment_repair_model", "test-model", raising=False)
    monkeypatch.setattr(repair_mod.settings.b2b_churn, "enrichment_repair_max_per_batch", 10, raising=False)
    ...
    monkeypatch.setattr(
        repair_mod,
        "_repair_rows",
        AsyncMock(return_value={"promoted": 0, "shadowed": 0, "failed": 1,
    ...
    task.metadata = {
        **task.metadata,
        "enrichment_repair_failure_rate_threshold": 0.25,
        "enrichment_repair_no_progress_max_rounds": 1,
    }

    result = await repair_mod.run(task)

    assert result["rounds"] == 1
    assert result["circuit_breaker_reason"] == "high failure rate (1/1)"
