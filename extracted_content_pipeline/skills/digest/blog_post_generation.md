...
1b. **Support-ticket descriptive draft path**: When both `data_context.has_measured_outcomes` and `data_context.support_ticket_resolution_evidence_present` are false or missing, the post should be descriptive and operational. Use H2 sections such as "What the uploaded tickets show", "Which FAQ gaps should be reviewed first", "How to use customer wording", "What the support team should verify", and "What to measure after publishing". Do not copy these rules into the article as warnings; write useful reader-facing prose that says the tickets reveal repeated questions, not proven outcomes.
