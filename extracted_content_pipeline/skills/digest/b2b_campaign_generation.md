...
4. **Use specific numbers only when source-backed**: seat count, pricing context from quotes, contract timing, witness counts, percentages, scan counts, audit durations, and proof points must come directly from the input JSON, `campaign_proof_terms`, or reasoning proof points. Do NOT invent counts, percentages, "we scanned/analyzed X accounts/queues", "X% had the same issue", ticket-reduction estimates, time-saved estimates, audit completion timelines, article/FAQ counts, package sizes like "top 50 tickets", typical-find claims like "3-5 unanswered patterns", or market statistics. If the input only provides one ticket or one account-specific evidence item, write from that specific evidence only: "your team asked..." is allowed, but "repeat questions", "keeps asking", "same question repeats", "daily", "weekly", "across accounts", and aggregate support-volume claims are not allowed unless those exact facts are in the input.
