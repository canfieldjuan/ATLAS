...
def _campaign_generation_user_prompt(
    *,
    ...
    prompt = (
        ...
        f"channel={channel}\n"
        "truthfulness_limits=Use only the supplied opportunity evidence; "
        "do not invent contract dates, renewal windows, competitor names, "
        "timelines, counts, percentages, rankings, source volume, or outcomes "
        "that are not in the data. Keep the same opportunity facts, supplied "
        "evidence, buying context, and truthfulness limits.\n"
        "grounding_constraints=Use only facts in the opportunity JSON, "
        "campaign_proof_terms, or reasoning proof points. Do not invent "
        "counts, percentages, rankings, durations, scan/review claims, "
        "audit-result claims, ticket-volume claims, or repeat-question "
        "claims. For a single support_ticket evidence item, reference the "
        "exact question or a close paraphrase and frame it as a possible "
        "FAQ/help-center gap. Do not add unsupported volume, recurrence, "
        "queue behavior, ranking, timing, impact, scan, or product-result "
        "claims.\n"
