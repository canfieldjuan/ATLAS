...
are unioned. Determinism holds because the model name + revision and the threshold are
pinned, inference is CPU, and iteration order is fixed. Auditability is preserved by
...
- The implementation PR (follow-up). Proposed file set:
  ...
  `tests/test_extracted_ticket_faq_markdown.py`. Estimated ~370 code LOC; if over the
  400 cap, split at the port boundary (Slice A floor + seam, Slice B booster + adapter).
...
- The implementation PR will add cases in `tests/test_extracted_ticket_faq_markdown.py`
  ...
  reworded-pair merge with cosine, determinism, merge-provenance) and run
