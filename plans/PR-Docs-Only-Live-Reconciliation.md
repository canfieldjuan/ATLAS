...
- Root cause: `scripts/check_ai_reconciliation_live.py` had no trusted
  changed-file proof for the docs-only attestation bypass. The PR body contract
  checks changed paths separately, but that context is not a required merge gate,
  so `live-reconciliation` must not trust the marker by itself.
- Correct fix must touch/change: require both the `Docs-only: true` body marker
  and a non-empty Markdown-only GitHub changed-file list before bypassing
  current-head review attestation; test docs-only/no-thread/Markdown-only pass
  plus docs-only/non-Markdown, docs-only/open-thread, and non-doc/no-review fail.
...
- Acceptance criteria:
  - `scripts/check_ai_reconciliation_live.py` returns success for a body whose
    ...
    live changed-file list is non-empty Markdown-only.
  - The same docs-only body still fails for a non-Markdown changed-file list
    unless current-head Codex review/clean-comment attestation is present.
...
- Risk areas: over-broad docs-only detection, trusting PR body text without
  changed-file proof, accidentally clearing open Codex threads, accidentally
  clearing `CHANGES_REQUESTED`, and weakening non-doc PR review freshness.
...
- Replaced-path behaviors: current behavior requires review attestation for all
  ...
  body is explicitly docs-only, the PR changed-file list is Markdown-only, and
  there are no open Codex threads."
- Guard-relevant fields: PR body first nonblank line, GitHub PR changed-file
  filenames/previous filenames, unresolved Codex thread set, current-head Codex
  review states, and clean-review comments.
...
- Explicit value probe: tests use the default exact Codex identity set, an
  explicit `Docs-only: true` marker, and Markdown-only changed-file fixtures.
...
fetches the PR's changed-file list from GitHub and treats the exemption as valid
only when every current filename and rename source has Markdown as its sole
suffix. `evaluate()` still computes current-head change requests and unresolved
Codex threads first. If neither blocking condition exists and both docs-only
proofs pass, it returns success without requiring a Codex review attestation.
...
- Do not exempt arbitrary marker-only PRs; the explicit body marker and the live
  Markdown-only changed-file proof must both be present.
...
- `python -m pytest tests/test_check_ai_reconciliation_live.py -q` - 49 passed.
...
- `python scripts/audit_plan_doc_diff_size.py plans/PR-Docs-Only-Live-Reconciliation.md origin/main` - passed with warning, estimate 398 actual 237.
...
|---|---:|
| `plans/INDEX.md` | 4 |
| `plans/PR-Docs-Only-Live-Reconciliation.md` | 160 |
| `plans/archive/PR-Codex-Review-Scope-Reset.md` | 0 |
| `plans/archive/PR-Codex-Review-Thread-Event-Canary.md` | 0 |
| `scripts/check_ai_reconciliation_live.py` | 92 |
| `tests/test_check_ai_reconciliation_live.py` | 142 |
| **Total** | **398** |
