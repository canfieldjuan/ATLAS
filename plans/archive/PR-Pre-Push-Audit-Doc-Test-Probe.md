...
- Acceptance criteria:
  ...
  - The checker parses block-style and flow-style branch filters before
    ...
    ordered negation and simple glob patterns, and fails closed on unsupported
    GitHub glob syntax (`+`, `?`, character classes, escapes) instead of
    accepting Python `fnmatch` semantics as equivalent.
