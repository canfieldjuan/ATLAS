...
- Acceptance criteria:
  ...
  - `scripts/run_eom_lead_pipeline_checks.sh` contains the same pytest files
    that `.github/workflows/atlas_eom_lead_pipeline_checks.yml` enforces on
    current main; workflow rewiring is deferred because #2216 touches that
    workflow.
...
warns unless the Verification section includes both markers. The EOM lead
pipeline workflow's existing pytest list is mirrored in
`scripts/run_eom_lead_pipeline_checks.sh`, making the enforced set a single
local command without editing #2216's in-flight workflow file.
...
- The detector checks plan text, not whether a human actually ran the command.
...
- python -m pytest tests/test_check_enforced_set_verification.py -q --noconftest - 10 passed.
- bash scripts/run_eom_lead_pipeline_checks.sh - 189 passed, 8 skipped, 1 warning.
...
| `scripts/check_enforced_set_verification.py` | +136 |
| `tests/test_check_enforced_set_verification.py` | +106 |
| `scripts/run_eom_lead_pipeline_checks.sh` | +18 |
| `.github/workflows/enforced_set_verification.yml` | +46 |
| `plans/PR-Enforced-Set-Verification.md` | +132 |
| **Total** | **~465** |
