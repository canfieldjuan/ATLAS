...
4. Add `tests/test_invoice_payment_reminders_disabled.py` (9 tests) pinning all three layers, including that `reminders_enabled=True` still produces no send and that `run()` touches no config/repository/transport modules before the guard.
5. Keep the two existing send-shape tests documenting the behaviour #2271 must revive by calling the extracted `_send_due_reminders(task)` seam directly; they do not patch the kill-switch constant.
...
- Risk areas: guard placement relative to side effects; the config default flip reaching an unintended consumer of `reminders_enabled`; the seed edit perturbing other `_DEFAULT_TASKS` entries or the seeding loop; the extracted `_send_due_reminders(task)` seam accidentally becoming a scheduler entrypoint instead of remaining test/documentation support for #2271.
...
- Side-effect ordering: the guard is the first statement in `run()`. `sent_emails`, `contact_interactions` and `repo.update_reminder` all live below it and are unreachable. The permitted-side tests call `_send_due_reminders(task)` directly and never set `_AUTOPILOT_DISABLED=False`, so no test can leak a disabled kill switch into another test.
