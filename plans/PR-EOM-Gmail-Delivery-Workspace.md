...
- Guard-relevant fields: `billing_run_id`, `limit`, and `offset`; the route validates query bounds and the service repeats bounds for direct callers.
...
- `lifecycle_conflict` is visible rather than repaired automatically; corrections remain explicit audited actions in their existing writers.
...
- Focused durable delivery-state service/route proof: `20 passed` after the beyond-end offset-total, same-snapshot pagination, stale-PDF, and unfinished-draft recovery regression repairs.
- Existing commercial Gmail draft and sent-reconciliation suite: `106 passed`.
...
| `atlas_brain/services/commercial_billing_invoice_gmail_sent_reconciliation.py` | 506 |
| `atlas_brain/services/commercial_billing_invoice_pdfs.py` | 32 |
| `plans/PR-EOM-Gmail-Delivery-Workspace.md` | 113 |
| `tests/test_commercial_billing_gmail_drafts.py` | 548 |
| **Total** | **1233** |
