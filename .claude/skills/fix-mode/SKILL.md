...
baseline alone. The hook writes `upstream_edit_receipts` after an admitted
upstream-only edit so downstream edits can prove a current-pass source touch.
Also mirror these fields into the `## PR Fix Mode` block of
`SESSION_STATE.local.md` (the human baton).
