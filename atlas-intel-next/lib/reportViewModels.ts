...
function formatWeaknessArea(area: string): string {
  return area.replace(/_/g, ' ').replace(/\b\w/g, (c) => c.toUpperCase())
}

export function toWeaknessAnalysis(value: unknown): WeaknessAnalysisItemViewModel[] {
  return toRecordArray(value).map((item) => {
    const rawArea = asString(item.area)
    const weaknessLabel = asString(item.weakness) ?? (rawArea ? formatWeaknessArea(rawArea) : undefined)
    const evidenceCount = asNumber(item.evidence_count) ?? null
    // Build evidence text from count when no explicit evidence string
    const evidenceText = asString(item.evidence)
      ?? (evidenceCount != null ? `${evidenceCount} supporting review${evidenceCount === 1 ? '' : 's'}` : undefined)
    return {
