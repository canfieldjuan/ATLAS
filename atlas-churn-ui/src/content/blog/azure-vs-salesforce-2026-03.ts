...
<p><strong>Pricing transparency</strong> is the second most common pain category for Salesforce. Reviewers cite confusion around licensing tiers, add-on costs, and feature gating. One reviewer reported being quoted one price by the sales team, then receiving an invoice 40% higher after add-ons they hadn't known were required. The urgency score for pricing complaints is lower than Azure's, but the frequency is comparable.</p>
...
<p>Salesforce reviewers consistently praise <strong>ecosystem maturity</strong> and <strong>third-party app availability</strong>. One verified reviewer on G2 writes: "Whatever we need, there's an AppExchange app for it. The ecosystem is unmatched." Another reviewer noted that Salesforce's reporting tools are powerful once learned — the initial setup is painful, but the payoff is worth it.</p>
