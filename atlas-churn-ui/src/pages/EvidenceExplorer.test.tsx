...
describe('EvidenceExplorer', () => {
  ...
  it('opens and copies a company-scoped alerts link from a witness card', async () => {
    const user = userEvent.setup()
    const clipboardSpy = vi.spyOn(window.navigator.clipboard, 'writeText').mockResolvedValue(undefined)
    api.fetchWitnesses.mockResolvedValueOnce({
      vendor_name: 'Zendesk',
    ...
    const expectedPath = '/alerts?vendor=Zendesk&company=Acme+Corp&back_to=%2Fevidence%3Fvendor%3DZendesk%26tab%3Dwitnesses%26source%3Dreddit%26offset%3D30%26witness_id%3Dwitness%253Azendesk%253A31'

    expect(await screen.findByDisplayValue('Zendesk')).toBeInTheDocument()
    expect(screen.getByRole('link', { name: 'Open alerts for witness witness:zendesk:31' })).toHaveAttribute('href', expectedPath)
    await user.click(screen.getByRole('button', { name: 'Copy alerts link for witness witness:zendesk:31' }))

    await waitFor(() => {
      expect(clipboardSpy).toHaveBeenCalledWith(`${window.location.origin}${expectedPath}`)
    })
    expect(screen.getByRole('button', { name: 'Copy alerts link for witness witness:zendesk:31' })).toHaveTextContent('Copied')
  })
