...
function CampaignReasoningSummary({ draft }: { draft: ReviewQueueDraft }) {
  const scope = draft.reasoning_scope_summary
  const atom = draft.reasoning_atom_context
  const delta = draft.reasoning_delta_summary
  ...
  const deltaSignals = [
    ...(delta?.theses_added ?? []).map((item) => `new thesis: ${item}`),
    ...(delta?.new_timing_windows ?? []).map((item) => `new timing: ${item}`),
    ...(delta?.new_account_signals ?? []).map((item) => `new account: ${item}`),
  ]
          ...
          {timingWindows.length > 0 && (
            <div className="rounded-lg border border-slate-700/40 bg-slate-900/40 p-3">
              ...
              <div className="mt-2 space-y-2">
                {timingWindows.slice(0, 2).map((item) => (
                  <div key={`${item.window_type}:${item.anchor}`} className="text-xs text-slate-300">
                    <p className="font-medium text-slate-200">{item.anchor}</p>
                    <p className="text-slate-400">
                      {[item.window_type, item.urgency].filter(Boolean).join(' | ')}
                    </p>
                    {item.recommended_action && (
                      <p className="mt-1 text-slate-400">{item.recommended_action}</p>
                    )}
                  </div>
                ))}
              </div>
              ...
              <div className="mt-2 space-y-2">
                {proofPoints.slice(0, 2).map((item) => (
                  <div key={item.label} className="text-xs text-slate-300">
                    ...
                    {item.value != null && (
                      <p className="text-slate-400">{String(item.value)}</p>
                    )}
                    {item.interpretation && (
                      <p className="mt-1 text-slate-400">{item.interpretation}</p>
                    )}
                  </div>
                ))}
              </div>
            ...
            {accountSignals.slice(0, 2).map((signal, index) => (
              <div key={`${signal.company || 'account'}:${index}`} className="rounded-lg border border-slate-700/30 bg-slate-800/40 p-3 text-xs text-slate-300">
                <div className="flex items-center justify-between gap-2">
                  <p className="font-medium text-slate-100">{signal.company || draft.company_name}</p>
                  {signal.trust_tier && (
      ...
      {deltaSignals.length > 0 && (
        <div className="mt-3 rounded-lg border border-slate-700/40 bg-slate-900/40 p-3">
          ...
          <div className="mt-2 flex flex-wrap gap-2">
            {deltaSignals.slice(0, 4).map((item) => (
              <span key={item} className="rounded-full bg-cyan-500/10 px-2 py-0.5 text-[11px] text-cyan-200">
                {item}
              </span>
            ))}
          </div>
                      ...
                      <div key={draft.id} className="p-4">
                        <div className="flex items-start gap-3">
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            {/* Email preview */}
                            <CampaignReasoningSummary draft={draft} />
