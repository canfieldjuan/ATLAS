...
describe('IncidentAlerts', () => {
  ...
  it('shows other recent exact targets in the vendor-scoped header', async () => {
    api.listWebhooks.mockResolvedValueOnce({
      webhooks: [
        {
          id: 'wh-report',
    ...
    const region = await screen.findByRole('region', { name: 'Other recent exact targets' })
    expect(within(region).getByText('Report target: Older Battle Card')).toBeInTheDocument()
    expect(within(region).getByText(/Latest failure · report_generated · 500/)).toBeInTheDocument()
    expect(within(region).getByText(/Report delivery/)).toBeInTheDocument()
    expect(within(region).getByRole('link', { name: 'Open Report' })).toHaveAttribute(
      'href',
      '/reports/report-older?back_to=%2Falerts%3Fvendor%3DAcme%2520Rival',
    )
  })

  it('shows an exact target summary card for vendor-scoped views', async () => {
    api.listWebhooks.mockResolvedValueOnce({
      webhooks: [
        {
          id: 'wh-report',
    ...
    expect(exactTargetsRegion.getByText('2')).toBeInTheDocument()
  ...
  it('does not show a latest exact target header region for vendor-only activity', async () => {
    ...
    expect(screen.queryByText('Exact Targets')).not.toBeInTheDocument()
  })
