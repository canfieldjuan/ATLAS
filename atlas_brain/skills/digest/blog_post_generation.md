...
1b. **Support-ticket descriptive draft path**: When `data_context.support_ticket_blog_mode` is `descriptive_no_outcome`, the post must follow the explicit contract fields in the blueprint. Build the article from `allowed_claims`, avoid every item in `forbidden_claims`, use `required_section_outline` as the H2 order, and use `draft_faq_shells` for every FAQ answer shell. Each shell answer must stay review-needed using `draft_answer_guidance`; do not invent a richer answer body. When both `data_context.has_measured_outcomes` and `data_context.support_ticket_resolution_evidence_present` are false or missing, the post should be descriptive and operational. If `data_context.source_row_count` or `data_context.included_ticket_row_count` is 25 or fewer, write a short FAQ review brief instead of a full article: 700-1100 words, 3-4 H2 sections total, no H3 subsections, no broad scaling/process section, and no repeated sections that explain the same cluster, FAQ gap, or customer wording twice. The first two H2 sections must open with 40-80 word answer paragraphs that include the exact `target_keyword` when provided, or the clear support-ticket subject when no target keyword is provided. Do not add extra benefit, impact, search, or self-service sections beyond the observed shell. Do not copy these rules into the article as warnings; write useful reader-facing prose that says the tickets reveal repeated questions, not proven outcomes.
...
7. **Length**: 1500-2200 words for the main content. Exception: for the small-upload support-ticket descriptive draft path in rule 1b, use 700-1100 words, 3-4 H2 sections, and no H3 subsections. Concise paragraphs (2-4 sentences each).
