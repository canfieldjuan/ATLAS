...
Uses the existing CalendarTool for API access.
"""
...
from ..tools.calendar import CALENDAR_API_BASE, TOKEN_URL
...
class Appointment:
    ...
    context_id: Optional[str] = None  # Business context
...
class SchedulingService:
    ...
    def _get_day_slots(
        ...
        open_time = getattr(context.hours, f"{day_name}_open")
        close_time = getattr(context.hours, f"{day_name}_close")
        ...
        while slot_start + timedelta(minutes=duration) <= day_end:
            ...
            # Check if slot is in the future with required notice
            if slot_start >= min_start:
                # Check if slot conflicts with busy times (including buffer)
    ...
    async def book_appointment(
        ...
        # Build event summary
        summary = f"{context.name} - {customer_name}"
        if service_type:
        ...
        url = f"{CALENDAR_API_BASE}/calendars/{context.scheduling.calendar_id}/events"
    ...
    async def cancel_appointment(
        ...
        url = f"{CALENDAR_API_BASE}/calendars/{context.scheduling.calendar_id}/events/{appointment_id}"
    ...
    def format_slots_for_speech(
        ...
        if len(slots) == 1:
            return f"I have one opening: {slots[0]}"
        ...
        for day, day_slots in by_day.items():
            times = [s.start.strftime("%-I:%M %p") for s in day_slots[:3]]
            if len(times) > 1:
                times_str = ", ".join(times[:-1]) + " or " + times[-1]
            else:
                times_str = times[0]
            parts.append(f"{day}: {times_str}")

        if len(slots) > max_slots:
            parts.append(f"I have {len(slots) - max_slots} more openings if those don't work.")
