...
def normalized_company_name_sql(column_sql: str) -> str:
    ...
    # Use E'...' escape string for _TRAILING_PUNCT_PATTERN because it ends with
    # a regex $ anchor that collides with any dollar-quote delimiter.
    # The other patterns have no $ so $$ is safe for them.
    trailing_escaped = _TRAILING_PUNCT_PATTERN.replace("\\", "\\\\")
    return (
        "TRIM("
        "REGEXP_REPLACE("
        "REGEXP_REPLACE("
